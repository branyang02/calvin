    ...
    def reset(self):
        if self.episode_counter is None:  # this is the first time reset has been called
            ...
            episode_log_dir = os.path.join(
                self.log_dir, "ep" + str(self.episode_counter)
            )
            ...
            # Log the actions
            np.save(
                os.path.join(episode_log_dir, "actions.npy"), np.array(self.action_seq)
            )
...
def evaluate_policy(
    model, env, epoch=0, eval_log_dir=None, debug=False, create_plan_tsne=False
):
    """
    ...
    """
    conf_dir = Path(__file__).absolute().parents[2] / "calvin-sim/calvin_models/conf"
